...
def load_data():
    ...
    data[['HomeTeam','AwayTeam']] = data[['HomeTeam',
                                          'AwayTeam']].apply(pd.to_numeric)
    # for some reason, goals are not additionally counted as shots on target
    data.loc[:, 'HST'] = data['HST'] + data['FTHG']
    data.loc[:, 'AST'] = data['AST'] + data['FTAG']
    # similarly, shots on target are not classed as shots
    data.loc[:, 'HS'] = data['HST'] + data['HS']
    data.loc[:, 'AS'] = data['AST'] + data['AS']
    return data, prior_data, teams

def fit_model(model_file):
    ...
    pl_data = {'nteams' : len(teams),
               ...
               'home_s': data['HS'].values,
               'away_s': data['AS'].values,
...
def plot_alpha_beta(a, b, teams):
    ...
    plt.errorbar(
        1.25,
        1.3,
        xerr=np.mean(np.std(a,axis=0)),
        yerr=np.mean(np.std(b,axis=0)),
        fmt='o', capthick=0.01
    )
    plt.gca().arrow(
        1.44, 1.64, 0., -0.2, head_width=0.02, head_length=0.02, fc='k', ec='k'
    )
    plt.text(1.38, 1.65, "Better \ndefense", size=7)
    plt.gca().arrow(
        0.6, 0.8, 0.2, 0., head_width=0.02, head_length=0.02, fc='k', ec='k'
    )
